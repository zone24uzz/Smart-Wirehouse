...
### Noutbukdan vaqtincha global ko‘rsatish

Cloudflare Quick Tunnel bilan `client/` dagi Vite sahifasini HTTPS orqali vaqtincha ochish mumkin. `client/.env.local` ga `VITE_API_URL=/api` yozing (Vite konfiguratsiyasidagi proxy so‘rovlarni `localhost:5000` ga uzatadi), frontend va backendni ishga tushiring, so‘ng `cloudflared tunnel --url http://127.0.0.1:5173` buyrug‘ini bajaring. Chiqqan `https://….trycloudflare.com` manzilini `server/.env` dagi `TELEGRAM_WEB_APP_URL` ga kiriting va backendni qayta ishga tushiring. URL har yangi quick tunnel ishga tushganda o‘zgarishi mumkin; tunnel, frontend, backend va noutbuk ishlayotgan hamda internet ulangan bo‘lishi kerak. Bu vaqtinchalik ko‘rsatish uchun; doimiy URL/deploy uchun Vercel va Render sozlamalaridan foydalaning.
